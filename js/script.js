...
// ---- Oneko Cat (Professional) ----
!function(){
    const el=document.getElementById('oneko');if(!el)return;
    let mx=innerWidth/2,my=innerHeight/2,ox=mx,oy=my;
    let frame=0,fTimer=0,dir='down',moving=false;
    const FW=64,FH=64,COLS=4;
    let idleTimer=0,blinkFrame=0,isBlinking=false;
    ...
    function tick(){
        ...
        if(moving){
            ...
            // Walk animation
            fTimer++;
            if(fTimer>5){frame=(frame+1)%4;fTimer=0}

            // Map to sprite row
            let row=1; // walk row
            if(dist>120)row=2; // run row
            ...
            // Idle - subtle breathing + blink
            idleTimer++;
            if(idleTimer>40){
                if(!isBlinking&&Math.random()>.6){
                    isBlinking=true;
                    blinkFrame=0;
                }
                idleTimer=0;
            ...
            // Use blink frame if blinking
            let showFrame=isBlinking?Math.min(blinkFrame,1):frame;
            let bgX=-(showFrame*FW);
            let bgY=-(0*FH); // idle row
            el.style.backgroundPosition=`${bgX}px ${bgY}px`;
        }

        // Position
        el.style.left=`${ox-FW/2}px`;
        el.style.top=`${oy-FH/2}px`;
